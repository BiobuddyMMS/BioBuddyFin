...
const gameRooms = {}; // ที่เก็บสถานะของห้องเกม (2 คน)
const soloSessions = {}; // ที่เก็บสถานะของโหมดฝึกเล่น (1 คน)

// ฟังก์ชันสำหรับหาห้องเกมที่ผู้ใช้คนนี้อยู่
...
/**
 * แนะนำคำถามแบบชาญฉลาด
 * @param {string[]} remainingAnimals - รายชื่อสัตว์ที่เหลือของทีมที่กำลังจะถาม
 * @returns {object|null} - object คำถาม {trait, value}
 */
...
async function handleEvent(event) {
    ...
    if (userMessage.match(/จบเกม/i)) {
        ...
        return client.replyMessage(event.replyToken, { type: 'text', text: "โอเค ออกจากเกมแล้ว! อยากเล่นใหม่ก็บอกนะเพื่อนซี้ 😉" });
    }
    ...
    if (soloSession) {
        ...
        if (guessMatch) {
            ...
            if (guessedAnimal === soloSession.secretAnimal) {
                ...
                delete soloSessions[userId];
                return client.replyMessage(event.replyToken, { type: 'text', text: winText });
            } else {
                return client.replyMessage(event.replyToken, { type: 'text', text: `ยังไม่ใช่! ลองอีกทีนะ 🤔` });
            }
        }
        return client.replyMessage(event.replyToken, { type: 'text', text: `ตอนนี้อยู่ในโหมดฝึกเล่นนะ! ถามคำถามเกี่ยวกับสัตว์ที่ฉันคิดไว้ หรือ "ทาย" ได้เลย!` });
    }
    ...
    if (room) {
        ...
        if (room.state === 'rolling') {
            if (userMessage.match(/(ทอยเต๋า|ทอยลูกเต๋า|วัดดวง)/i)) {
                ...
                await client.replyMessage(event.replyToken, { type: 'text', text: `ทีม ${room.players[userTeam].name} ทอยได้ ${diceRoll} แต้ม!` });
                ...
                if (room.players.red.hasRolled && room.players.blue.hasRolled) {
                    ...
                    room.state = 'playing';
                    await client.pushMessage(room.players.red.id, { type: 'text', text: resultText });
                    await client.pushMessage(room.players.blue.id, { type: 'text', text: resultText });
                } else {
                    await client.pushMessage(room.players[opponentTeam].id, { type: 'text', text: `ทีม ${room.players[userTeam].name} ทอยเต๋าแล้ว! ถึงตาเธอแล้วนะ!` });
                }
            }
        }
        if (room.state === 'playing') {
            ...
            const handleYesNoAnswer = async (isYes) => {
                ...
                // Build the detailed reply message
                let replyToCurrentPlayer = `โอเค! รับทราบคำตอบนะ\n\n`;
                const listLimit = 8; // Show max 8 animals per list

                // 1. Show eliminated animals
                if (eliminatedAnimals.length > 0) {
                    replyToCurrentPlayer += `❌ สัตว์ที่ถูกตัดออก (${eliminatedAnimals.length} ตัว):\n`;
                    if (eliminatedAnimals.length > listLimit) {
                        replyToCurrentPlayer += `- ${eliminatedAnimals.slice(0, listLimit).join('\n- ')}\n- และอีก ${eliminatedAnimals.length - listLimit} ตัว\n\n`;
                    } else {
                        replyToCurrentPlayer += `- ${eliminatedAnimals.join('\n- ')}\n\n`;
                    }
                } else {
                    replyToCurrentPlayer += `รอบนี้ไม่มีสัตว์ถูกตัดออกเลย\n\n`;
                }

                // 2. Show remaining animals
                if (animalsAfter.length > 0) {
                    ...
                    if (animalsAfter.length > listLimit) {
                        replyToCurrentPlayer += `- ${animalsAfter.slice(0, listLimit).join('\n- ')}\n- และอีก ${animalsAfter.length - listLimit} ตัว`;
                    } else {
                        replyToCurrentPlayer += `- ${animalsAfter.join('\n- ')}`;
                    }
            ...
            if (userMessage.match(/^(ใช่|ใช่เลย|ถูกต้อง|แม่นแล้ว|y|yes)$/i)) {
                return handleYesNoAnswer(true);
            } else if (userMessage.match(/^(ไม่|ไม่ใช่|ผิด|n|no)$/i)) {
                return handleYesNoAnswer(false);
            }
            ...
            if (guessMatch) {
                ...
                if (guessedAnimal === room.players[opponentTeam].secretAnimal) {
                    ...
                    await client.pushMessage(room.players.red.id, { type: 'text', text: winText });
                    await client.pushMessage(room.players.blue.id, { type: 'text', text: winText });
                    delete gameRooms[room.id];
                } else {
                    room.turnBonus = 1;
                    room.currentTurn = opponentTeam;
    ...
    if (!room && !soloSession) {
        ...
        if (userMessage.match(/(สร้างเกม|สร้างห้อง|เริ่มเกม|สร้างตี้)/i)) {
        ...
        // Default message with Quick Reply buttons
        const welcomeMessage = {
            type: 'text',
